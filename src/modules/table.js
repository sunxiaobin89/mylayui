...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
  ...
  var tableIns = {}, // 缓存table.render返回的实例对象
    ...
    tableSpecialColType = ['numbers', 'checkbox', 'radio', 'space'], // 表格的特殊类型字段// 获得table的config
    ...
    tableCheck = function () {
      ...
      return {
        ...
        set: function (tableId, type, data) {
          ...
          checked[tableId][type] = (!data || !layui._isArray(data)) ? [] : data;
        },
  ...
  table.reverse = function (tableId, reversal, tableIns) {
    if (!tableId && !tableIns) {
      ...
      if (!config) {
        hint.error('Table Ins not found: ' + tableIns || tableId);
        return;
      }
  ...
  var enableTableFixedScroll = function (enabled) {
    ...
    if (enabled) {
      ...
      _DOC.on('mouseenter', '.' + ELEM_VIEW + ' ' + ELEM_FIXR + ' ' + ELEM_BODY, function (event) {
        ...
        if (elemFixedR.hasClass(FIXED_SCROLL)) {
          return;
        }
        // if (elemFixedR.css('right') !== '-1px') {
        if (Number.parseFloat(elemFixedR.css('right')) > -1) {
          // 如果有滚动条的话
  ...
  tableCheck['CHECK_TYPE_ADDITIONAL'] = CHECK_TYPE_ADDITIONAL;
  tableCheck['CHECK_TYPE_REMOVED'] = CHECK_TYPE_REMOVED;
  tableCheck['CHECK_TYPE_ORIGINAL'] = CHECK_TYPE_ORIGINAL;
  tableCheck['CHECK_TYPE_DISABLED'] = CHECK_TYPE_DISABLED;
  ...
  Class.prototype.setInit = function (type) {
    ...
    if (options.checkDisabled && layui._isArray(options.checkDisabled.data) && options.checkDisabled.data.length) {
      tableCheck.disabled(tableId, layui._isArray(options.checkDisabled.data) ? options.checkDisabled.data : []);
    }
  ...
  Class.prototype.setColsWidth = function () {
    ...
    if (options.cols.length > 1) {
      ...
      var thBox = that.layBox.children(ELEM_HEADER);
      ...
      thElem.each(function (index, thCurr) {
        ...
        // if (rowspan < colsNum) {
        var thHeight = thBox.find('th[data-key="' + thCurr.attr('data-key') + '"]').height();
        thCurr.height('auto').find('>div.layui-table-cell').css({
          height: thHeight + 'px',
  ...
  Class.prototype.renderData = function (res, curr, count, sort, refresh) {
    ...
    //如果无数据
    if (data.length === 0) {
      return render(), that.errorView(options.text.none), renderDone.call(that, true);
    } else {
  ...
  Class.prototype.sort = function (th, type, pull, formEvent) {
    ...
    try {
      ...
      return hint.error('Table modules: sort field \'' + field + '\' not matched');
    }
